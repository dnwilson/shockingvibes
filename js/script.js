...
$(document).ready(function(e) {
	...
	* Hover text for the last slide
	*************************/
	$('#insta-block img').each(function(index, element) {
		...
		myImg.load(function(e) {
			img_loaded += 1;;
			if ( img_loaded == $('#insta-block img').length ) {
				$(function() {
					var pause = 10;
...
$(function() {
	...
	$(window).resize(function() {
		delay(function() {
				var gallery_images = $('#insta-block img');
				...
				var gallery_width = $('#insta-block img').width() * $('#insta-block img').length;
				gallery_width /= 2;
				if ( $('#insta-block img').length % 2 != 0 ) {
					gallery_width += $('#insta-block img').width();
				}
				
				$('#insta-block .row').css('width', gallery_width );
				
				var left_pos = $('#insta-block .row').width() - $('body').width();
				left_pos /= -2;
				
				$('#insta-block .row').css('left', left_pos);
...
jQuery(document).ready(function ($) {
	//Cache some variables
	var images = $('#insta-block a');
	
	images.hover(
		function(e) {
			var asta = $(this).find('img');
			$('#insta-block img').not( asta ).stop(false, false).animate(
				{
					opacity: .5
				},
				'fast',
		...
		function(e) {
			$('#insta-block img').stop(false, false).animate(
				{
					opacity: 1
				},
				'fast',
	...
	arrows.click(function(e) {
		...
		switch( $(this).attr('id') ) {
			...
			case 'arrow-down':
				...
				break;
			case 'arrow-left':
				offset_left = $('#insta-block .row').offset().left + 452;
				if ( offset_left > 0 ) {
					offset_left = '0px';
				}
				break;
			case 'arrow-right':
				offset_left = $('#insta-block .row').offset().left - 452;
				if ( offset_left < $('body').width() - $('#insta-block .row').width() ) {
					offset_left = $('body').width() - $('#insta-block .row').width();
				}
				break;
		}
		...
		if ( offset_left != false ) {
			if ( $('#insta-block .row').width() != $('body').width() ) {
				$('#insta-block .row').stop(false, false).animate({
					left: offset_left
				}, 1500, 'easeInOutQuart');
			}
		}
